...
function checkEligibility() {
  ...
  panel.className =
    "result-panel show " + (isEligible ? "approved" : "rejected");

  document.getElementById("resultIcon").textContent = isEligible ? "🎉" : "❌";
  document.getElementById("resultTitle").textContent = isEligible
    ? "Loan Approved"
    : "Loan Rejected";
  ...
  showToast(
    isEligible
      ...
      : "Application rejected based on rules.",
    isEligible ? "success" : "error",
  );
}
...
function updateRuleUI(elementId, isValid) {
  ...
  if (isValid) {
    ...
    el.innerHTML =
      ...
      el.textContent.replace(/^[✗✓●]|\(failed\)|\(passed\)/g, "").trim();
  } else {
...
async function getEligibilityAI() {
  ...
  const sysPrompt =
    "You are FinWise AI, an expert financial advisory system. Provide structured, detailed feedback on this applicant eligibility profile. Format using bullet points and short paragraphs. Be precise and suggest exact next steps.";
