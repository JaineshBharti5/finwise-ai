...
async function getAITips() {
  ...
  btn.disabled = true;
  btn.innerHTML =
    '<span class="spinner"></span> Running Llama 3.3 Financial Planner...';
  ...
  outputText.textContent =
    "Analyzing financial profile and planning portfolio...";
  ...
  if (response) {
    ...
    outputText.textContent =
      "Could not generate advice. Verify your API configuration in settings.";
  }
}
...
function loadHistory() {
  ...
  if (history.length === 0) {
    container.innerHTML =
      '<p style="color:var(--text-muted); font-size:12px; text-align:center;">No recent advisory sessions found.</p>';
    return;
  }
...
function saveAdvice() {
  ...
  const blob = new Blob(
    [
      `FINWISE AI FINANCIAL ADVISORY REPORT\nGenerated: ${new Date().toLocaleString()}\nQuery: ${lastAdviceQuery}\n\n${lastAdviceResponse}`,
    ],
    { type: "text/plain" },
  );
