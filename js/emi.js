...
function calculateEMI() {
  ...
  document.getElementById("breakInterest").textContent =
  ...
  document.getElementById("interestPct").textContent =
    `${interestPct.toFixed(1)}% Interest Split`;
  ...
  document.getElementById("aiBtn").innerHTML =
    '<i class="fas fa-brain"></i> Optimize with AI';
...
async function getEmiAI() {
  ...
  const sysPrompt =
    "You are FinWise AI, an expert loan auditor. Suggest strategies for prepayment, tenure adjustments and interest savings in India. Format with bullet points.";
  ...
  if (response) {
    ...
    aiText.textContent =
      "Could not generate optimization insights. Check Settings.";
  }
}
